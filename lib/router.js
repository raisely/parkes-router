...
class ParkesRouter extends KoaRouter {
	...
	resource(name, controller, methods, nestedFunc) {
		...
		const nestedMethods = ['index', 'create'];
		...
		const routes = {
			index: ['get', '/', controller.index, this.$$options.presentArray],
			show: ['get', `/:${itemName}`, controller.show, this.$$options.presentRecord],
			create: ['post', '/', controller.create, this.$$options.presentRecord],
			update: ['patch', `/:${itemName}`, controller.update, this.$$options.presentRecord],
			remove: ['delete', `/:${itemName}`, controller.remove, this.$$options.presentRecord],
		};
		...
		// Mount this resource
		this.use(`/${collectionName}`, resource.routes());
